...
**重要**：添加新类型后，需要同步更新以下文档位置：

#### README.md 中的文档表格（需要同步更新）

1. **资产类型表格**（第 343-352 行）
   - 在表格中添加新类型的行

2. **负债类型表格**（第 353-363 行）
   - 在表格中添加新类型的行

3. **代码注释中的类型列表**（第 118、126 行）
   - 更新注释中的类型名称列表

#### CLAUDE.md 中的文档表格（需要同步更新）

4. **资产类型表格**（第 178-187 行）
   - 在表格中添加新类型的行

5. **负债类型表格**（第 188-198 行）
   - 在表格中添加新类型的行

6. **代码示例注释**（第 158、166 行）
   - 更新注释中的类型名称列表

**示例**：添加"债券"类型后，需要在上述所有表格中添加对应的行。

**原因**：确保文档与代码保持一致，避免用户混淆。
...
路线图、战略决策与竞品/市场对标数据统一维护在 [ROADMAP.md](ROADMAP.md)（唯一事实来源），请以该文档为准。
